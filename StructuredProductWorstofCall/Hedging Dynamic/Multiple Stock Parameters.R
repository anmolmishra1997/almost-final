...
mu_all = array(0, dim = c(days,assets))
sigma_all = array(0, dim = c(days, assets))
...
i = 1
mu_all[1,] = as.numeric(input4[i, (2+assets):(1 + 2*assets)])
sigma_all[1,] = as.numeric(input4[i,2:(1 + assets)])
day_start = 2
day_end = 0
for ( i in 1:(dim(input4)[1]-1))
{
  sigma = as.numeric(input4[i,2:(1 + assets)])
  mu = as.numeric(input4[i, (2+assets):(1 + 2*assets)])
  ...
  mu_all[day_start:day_end,] = t(replicate(day_end - day_start + 1,mu ))
  sigma_all[day_start:day_end,] = t(replicate(day_end - day_start + 1, sigma))
...
if ( gamma == 0)
{
  for (a in 1:scenarios)
  {
    ...
    payoff_a <- Payoff_Function(hedge_prices_new, mu_all, value_initial, pr)
    ...
    for (k in 1:assets)
    {
      ...
      payoff_increment[,,k]<- Payoff_Function(hedge_prices_increment, mu_all,value_initial, pr)
...
if (gamma == 1)
{
  for (a in 1:scenarios)
  {
    ...
    for (k in 1:assets)
    {
      ...
      payoff_decrement[,,k]<- Payoff_Function(hedge_prices_decrement,mu_all, value_initial, pr)
...
roll_cost =  Roll_Cost(delta_all, mu_all)
